...
function Dashboard() {
	...
	const getRoutes = () => {
		return RouteAdminRole.map((data, key) => {
            if(data.routes && data.routes.length> 0){
                return data.routes.map(({second_path, component}, key) => {
                    return <Route path={second_path} element={component} key={key} />
                })
            }else{
                return <Route path={data.first_path} element={data.component} key={key} />
            }
		})
	}
	...
	return (
		<div className='flex flex-col md:flex-row w-full h-screen overflow-hidden font-poppins text-xs'>
			<Sidebar />
